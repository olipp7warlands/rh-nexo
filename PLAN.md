...
**Reclutamiento (VITAE)** — `Job`, `Candidate`, `Application`, `Stage`, `AuditDecision` (ya modelado en el prototipo VITAE; se formaliza en Fase 6).
...
**Objetivo:** incorporar VITAE como módulo y cerrar el embudo → plantilla.
**Acciones que funcionan:** flujo ATS funcional (ofertas, candidatos, pipeline, auto-selección con registro de auditoría); **al aceptar una oferta, se crea automáticamente el `Employee` y su `OnboardingProcess`** (la continuidad que ya se ve en el prototipo, hecha real).
**Datos:** `Job`, `Candidate`, `Application`, `AuditDecision` + enlace a `Employee`/`OnboardingProcess`.
